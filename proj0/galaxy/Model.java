...
class Model {
    ...
    /** The list of centers. */
    private List<Place> centers = new ArrayList<Place>();
    /** The array of marks. */
    private int [][] marks;
    /** The array of boundaries. */
    private boolean [][] boundaries;
    /** The variable # of columns. */
    private int column;
    /** The variable # of rows. */
    private int row;
    ...
    void copy(Model model) {
        ...
        for (int a = 0; a < model.xlim(); a++) {
            for (int b = 0; b < model.ylim(); b++) {
                this.marks[a][b] = model.marks[a][b];
            }
        }
        this.centers = new ArrayList<Place>();
        for (Place x:model.centers) {
            this.centers.add(x);
        }
        this.boundaries = new boolean[model.xlim()][model.ylim()];
        for (int a = 0; a < model.xlim(); a++) {
            for (int b = 0; b < model.ylim(); b++) {
                this.boundaries[a][b] = model.boundaries[a][b];
            }
        }
    }
    ...
    void init(int cols, int rows) {
        ...
        for (int x = 0; x < this.xlim(); x++) {
            for (int y = 0; y < this.ylim(); y++) {
                if (isEdge(x, y) && (x == 0 && y % 2 != 0
                        || y == 0 && x % 2 != 0)) {
                    this.boundaries[x][y] = true;
                } else if (isEdge(x, y) && ((x == this.column * 2)
                        || (y == this.row * 2))) {
                    this.boundaries[x][y] = true;
                }
            }
    ...
    boolean isEdge(int x, int y) {
        return (x >= 0 && x < xlim() && y >= 0 && y < ylim() && x % 2 != y % 2);
    }
    ...
    private boolean isGalaxy(Place center, HashSet<Place> region) {
        for (Place cell : region) {
            ...
            for (int i = 0; i < 4; i += 1) {
                ...
                if ((isCenter(intersection)) && ((intersection.x != center.x)
    ...
    HashSet<Place> findGalaxy(Place center) {
        ...
        if (isCell(center)) {
            ...
            for (Place a: asList(center.move(-1, 0), center.move(1, 0))) {
                accreteRegion(a, galaxy);
            }
        } else if (isHoriz(center)) {
            for (Place b: asList(center.move(0, -1), center.move(0, 1))) {
                accreteRegion(b, galaxy);
            }
        } else if (isIntersection(center)) {
            for (int i = 1; i < 4; i += 1) {
                int dx = 2 * (i / 2) - 1,
                        dy = 2 * (i % 2) - 1;
                for (Place c: asList(center.move(dx, dy))) {
    ...
    Set<Place> maxUnmarkedRegion(Place center) {
        ...
        ArrayList<Place> listregion =
                new ArrayList<Place>(unmarkedContaining(center));
        region.addAll(listregion);
        int length = 0;
        while (length < region.size()) {
            length = region.size();
            listregion.addAll(unmarkedSymAdjacent(center, listregion));
            region.addAll(listregion);
        }
        region.addAll(listregion);
        return region;
    ...
    void toggleBoundary(int x, int y) {
        if (isEdge(x, y)) {
            ...
            } else if (x < boundaries.length && x >= 0
                    && y < boundaries[0].length
                    && y >= 0) {
                boundaries[x][y] = true;
            }
        }
    ...
    int mark(int x, int y) {
        ...
        if (x < marks.length && x >= 0 && y < marks[0].length && y >= 0) {
            return marks[x][y];
        } else {
            return -1;
        }
    ...
    Place opposing(Place p0, Place p) {
        ...
        if (p0.x - distx >= 0 && p0.y - disty >= 0) {
            opp = pl(p0.x - distx, p0.y - disty);
        }
        return opp;
    }
    ...
    List<Place> unmarkedContaining(Place place) {
        if (isCell(place)) {
            ...
            for (int i = 0; i < 4; i += 1) {
                int dx = 2 * (i / 2) - 1,
                        dy = 2 * (i % 2) - 1;
                if ((marks[place.x + dx][place.y + dy] != 0)) {
                    n = false;
                }
            }
            if (n) {
    ...
    List<Place> unmarkedSymAdjacent(Place center, List<Place> region) {
        ...
        for (Place r : region) {
            assert isCell(r);
            for (int i = -2; i <= 2; i += 1) {
                int dx = 2 * (i / 2), dy = 2 * (i % 2);
                Place p = r.move(dx, dy);
                Place opp = opposing(center, p);
                if (isCell(p) && marks[p.x][p.y] == 0
                        && !result.contains(p)) {
                    if (opp != null && marks[opp.x][opp.y] == 0) {
                        result.add(p);
                    }
                }
            }
