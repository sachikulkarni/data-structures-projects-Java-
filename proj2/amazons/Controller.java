...
final class Controller {
    ...
    private Command[] _commands = {
        ...
        new Command("auto\\s([a-z]{5}).*$", this::doAuto),
        new Command("manual\\s([a-z]{5}).*$", this::doManual),
    };
    ...
    /** Make the move asked for by the player.
     * @param mat  */
    private void doMove(Matcher mat) {
        if (_board.isLegal(Move.mv(Square.sq(mat.group(1)),
                Square.sq(mat.group(2)), Square.sq(mat.group(3))))) {
            _board.makeMove(Square.sq(mat.group(1)),
                    Square.sq(mat.group(2)), Square.sq(mat.group(3)));
            if (!_board.legalMoves(_board.turn()).hasNext()) {
                _winner = board().turn().opponent();
                System.out.println();
                this.reportNote(_winner.toName() + " wins.");
            }

        }

    }
    /** Make the "white" or "black" an automated player.
     * @param mat  */
    private void doAuto(Matcher mat) {
        if (mat.group(1).equalsIgnoreCase("white")) {
            _white = _autoPlayerTemplate.create(WHITE, this);
        } else if (mat.group(1).equalsIgnoreCase("black")) {
            _black = _autoPlayerTemplate.create(BLACK, this);
        } else {
            throw error("invalid manual player");
        }
    }
    /** Make the "white" or "black" a human
     * player (entering moves as manual commands).
     * @param mat */
